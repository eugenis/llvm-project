...
bool FrameObjectCompare(const FrameObject &A, const FrameObject &B) {
  ...
  // The "first" object goes first (closest to SP).
  if (A.ObjectFirst != B.ObjectFirst)
    return B.ObjectFirst;

  // The "first" group members follow.
  if (A.GroupFirst != B.GroupFirst)
    return B.GroupFirst;

  // Order the rest by the group ID first to keep the groups together.
